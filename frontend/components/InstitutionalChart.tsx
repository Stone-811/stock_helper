...
/**
 * 三大法人趨勢圖（client 端 lazy 從 /api/stock/[id]/institutional 載入，不拖慢個股頁 TTFB）
 * 兩種模式：
...
export default function InstitutionalChart({ stockId, height = 300 }: { stockId: string; height?: number }) {
  ...
  const title = mode === 'holding' ? '外資持股張數' : '三大法人累計買賣超'
  const subtitle = mode === 'holding' ? '（張，實際持有量）' : '（張，區間起點歸零）'
  const tabBtn = (active: boolean, activeColor = 'bg-green-600') =>
    `px-2.5 md:px-3 py-1 md:py-1.5 text-sm font-medium rounded min-h-[40px] md:min-h-[36px] ${active ? `${activeColor} text-white` : 'bg-[#2a2a3e] text-white hover:bg-[#3a3a4e]'}`
