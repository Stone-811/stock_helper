 ...
 * 取得分析報告列表
 */
export async function getAnalysisReports(stockId?: string, limitCount: number = 10): Promise<any[]> {
  try {
    let queryRef = db.collection('stock_analysis_reports')
      .orderBy('created_at', 'desc')
      .limit(limitCount)

    if (stockId) {
      queryRef = db.collection('stock_analysis_reports')
        .where('stock_id', '==', stockId)
        .orderBy('created_at', 'desc')
        .limit(limitCount)
    }
 ...
 * 取得單一分析報告
 */
export async function getAnalysisReport(reportId: string): Promise<any | null> {
  try {
    const docRef = db.collection('stock_analysis_reports').doc(reportId)
...
export async function saveAnalysisReport(report: {
  ...
  try {
    const docRef = await db.collection('stock_analysis_reports').add({
      ...report,
      created_at: new Date()
    })
    return docRef.id
  } catch (error) {
    console.error('儲存分析報告失敗:', error)
    return null
  }
}
